...
def if_point_on_line(x1, y1, x2, y2, coord, buffer):
    #print(abs(distance((x1, y1),coord) + distance((x2, y2), coord) - distance((x1, y1), (x2, y2))))
    if int(abs(distance((x1, y1),coord)) + int(distance((x2, y2), coord)) - int(distance((x1, y1), (x2, y2)))) < 4:
        return True
    return False

def get_collision_angel(ball_angle, wall_angel):
    return(wall_angel-ball_angle)
...
class Laser:
    ...
    def update(self, keys, nodes, node_dist):
        ...
        if keys[self.bindings["ZAP"]]: self.color = (150, 25, 25)
        else: self.color = self.base_color
        #print(self.get_angel())
...
class Ball:
    def __init__(self, x, y, xv, yv, size, color):
        self.x, self.y, self.xv, self.yv= x, y, xv, yv
        self.size = size
        self.color = color

    def update(self, laser):
        if_point_on_line(laser.x1, laser.y1, laser.x2, laser.y2, (self.x, self.y), 0)

        self.x, self.y = pygame.mouse.get_pos()


    def bounce_off_line(self, laser, collision_point):
        recip = laser.get_angel()-math.pi/2
        self.xv = 10*math.cos(recip)
        self.yv = 10*math.sin(recip)

        
    def draw(self, surface, laser):
        

        collision_point1 = self.x-self.size/4 + self.size/2 * math.cos(laser.get_angel() - math.pi/2), self.y-self.size/4 + self.size/2 *  math.sin(laser.get_angel() - math.pi/2)
        collision_point2 = self.x-self.size/4 - self.size/2 * math.cos(laser.get_angel() - math.pi/2), self.y-self.size/4 - self.size/2 *  math.sin(laser.get_angel() - math.pi/2)

        if if_point_on_line(laser.x1, laser.y1, laser.x2, laser.y2, collision_point1, 0): 
            self.bounce_off_line(laser, collision_point1)
            print("yay")
        elif if_point_on_line(laser.x1, laser.y1, laser.x2, laser.y2, collision_point2, 0): 
            self.bounce_off_line(laser, collision_point1)
            print("yay")

        show_collision_points = False
        if show_collision_points == True:pygame.draw.ellipse(surface, (255, 0, 0), pygame.Rect(self.x-self.size/4 + math.cos(laser.get_angel() - math.pi/2), self.y-self.size/4 + self.size/2 *  math.sin(laser.get_angel() - math.pi/2), self.size//2, self.size//2))
        if show_collision_points == True:pygame.draw.ellipse(surface, (255, 0, 0), pygame.Rect(self.x-self.size/4 - math.cos(laser.get_angel() - math.pi/2), self.y-self.size/4 - self.size/2 *  math.sin(laser.get_angel() - math.pi/2), self.size//2, self.size//2))
...
balls = [Ball(random.randint(100, 700), random.randint(100, 700), random.randint(-3, 3), random.randint(-3, 3), random.randint(50, 100), (random.randint(35, 255),random.randint(35, 255), random.randint(35, 255))) for i in range(num_balls)]
shake_amount = 0
...
while 1:
    ...
    player.draw(surface)
    for ball in balls: ball.draw(surface, player.laser)
